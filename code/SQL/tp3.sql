...
FROM Persona, (SELECT codigoCliente
               FROM  PrefiereZona AS S0
               WHERE  NOT EXIST (SELECT nombreZona
                                 FROM Zona AS S1
                                 WHERE nombrePoblacion = "Rosario"  AND NOT EXIST (SELECT ALL
                                                                                   FROM PrefiereZona AS S2
                                                                                   WHERE S2.codigoCliente = S0.codigoCliente AND  S2.nombreZona = S1.nombreZona))) AS S4
WHERE Persona.codigo = S4.codigoCliente



SELECT Persona.nombre, Persona.apellido, Inmueble.codigo, Inmueble.nombreZona, Inmueble.precio
FROM   Persona, PrefiereZona, Limita, Inmueble
WHERE  Persona.codigo = PrefiereZona.codigoCliente AND /*Zonas preferidas*/
       (Limita.nombrePoblacion = PrefiereZona.nombrePoblacion AND /*Zonas limitrofes a las preferidas*/
       Limita.nombreZona = PrefiereZona.nombreZona AND
       Inmueble.nombrePoblacion = Limita.nombrePoblacionDos AND /*Inmuebles en zonas limitrofes*/
       Inmueble.nombreZona = Limita.nombreZonaDos
       OR
       Limita.nombrePoblacionDos = PrefiereZona.nombrePoblacion AND /*Zonas limitrofes a las preferidas*/
       Limita.nombreZonaDos = PrefiereZona.nombreZona AND
       Inmueble.nombrePoblacion = Limita.nombrePoblacion AND /*Inmuebles en zonas limitrofes*/
       Inmueble.nombreZona = Limita.nombreZona)

       AND
       Persona.codigo IN  (SELECT codigo
                           FROM Cliente
                          WHERE NOT EXIST (SELECT ALL
                                           FROM PrefiereZona,Inmueble
                                           WHERE Cliente.codigo = PrefiereZona.codigoCliente AND
                                                 Inmueble.nombrePoblacion = PrefiereZona.nombrePoblacion AND
                                                 Inmueble.nombreZona = PrefiereZona.nombreZona AND
                                                 NOT EXIST (SELECT ALL
                                                            FROM Visitas
                                                            WHERE Visitas.codigoCliente = Cliente.codigo AND
